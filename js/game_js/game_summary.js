...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    let gameSessionId = localStorage.getItem("gameSessionId"); //get game session id
...
async function checkCompleteOrOver(){
    let query=`SELECT game_completed, game_over FROM game_sessoins WHERE game_session_id=${gameSessionId}`;
    let gameCompleted = false;
    let gameOver = false;
    try{
        let result = DBQuery.getQueryResult(query);
...
async function setPlayerAllies(){
    ...
    for(let i=0; i<missingAllies.length; i++){
        allyElementId = missingAllies[i] + "-container";
        allyElement = document.getElementById(allyElementId);
...
async function setPlayerStats(){
    ...
    try{
        let result = DBQuery.getQueryResult(scoreRepQuery);
        ...
        console.error("An error occurred when getting the score & reputationt.");
        return;
    }
    ...
    try{
        let result = DBQuery.getQueryResult(itemsFoundQuery);
    ...
    try{
        let result = DBQuery.getQueryResult(areasVisitedQuery);
...
async function setPlayerMinigames(){
    ...
    try{
        let result = DBQuery.getQueryResult(minigamesPlayedQuery);
...
async function setGlobalStats(){
    ...
    try{
        let result = DBQuery.getQueryResult(avgScoreRepQuery);
        ...
        if(result.data[0].averageScore){
            //set value
            globalAvgReputationElement.innerHTML = `${Math.round(result.data[0].averageScore)}`;
        }
    ...
    let avgItemsCollectedQuery = `
    SELECT AVG(l.quantity) AS averageItemsCollected
    FROM game_session_logs_full_details AS l
    INNER JOIN game_sessions AS gs
    ON l.game_session_id = gs.game_session_id
    WHERE log_name="found_item" AND
    (game_over=1 OR game_completed=1);`;
    try{
        let result = DBQuery.getQueryResult(avgItemsCollectedQuery);
        ...
        console.error("An error occurred when getting the global avg items collected.");
        return;
    }

    //set the average areas visited
    let avgAreasVisitedQuery = `
    SELECT AVG(noAreasVisited) AS averageAreasVisited
    FROM (
        SELECT l.game_session_id, COUNT(log_id) AS noAreasVisited
        FROM game_session_logs_full_details AS l
        INNER JOIN game_sessions AS gs
        ON l.game_session_id = gs.game_session_id
        WHERE route_log = 1 AND
        (game_over=1 OR game_completed=1)
        GROUP BY l.game_session_id
    ) AS areas_visited_per_session;`;
    try{
        let result = DBQuery.getQueryResult(avgAreasVisitedQuery);
...
async function setGlobalMinigames(){
    ...
    let globalAvgMinigamesFailedElement = document.getElementById("minigames-failed-collected-global-avg");

    //set the average minigames played
    let avgMinigamesPlayedQuery = `
    SELECT AVG(l.quantity) AS averageMinigamesPlayed
    FROM game_session_logs_full_details AS l
    INNER JOIN game_sessions AS gs
    ON l.game_session_id = gs.game_session_id
    WHERE log_name="played_minigame" AND
    (game_over=1 OR game_completed=1);`;
    try{
        let result = DBQuery.getQueryResult(avgMinigamesPlayedQuery);
    ...
    let avgMinigamesWonQuery = `
    SELECT AVG(l.quantity) AS averageMinigamesWon
    FROM game_session_logs_full_details AS l
    INNER JOIN game_sessions AS gs
    ON l.game_session_id = gs.game_session_id
    ...
    //set the average minigames played
    let avgMinigamesFailedQuery = `
    SELECT AVG(l.quantity) AS averageMinigamesFailed
    FROM game_session_logs_full_details AS l
    INNER JOIN game_sessions AS gs
    ON l.game_session_id = gs.game_session_id
    WHERE log_name="minigame_failed" AND
    (game_over=1 OR game_completed=1);`;
    try{
        let result = DBQuery.getQueryResult(avgMinigamesFailedQuery);
